...
/// <summary>
/// Generators for the lowering oracle (VERIFICATION-MODEL.md section 7, ticket M2-003): straight-line
/// code plus <c>if</c>/<c>else</c> and early returns over <c>int</c>/<c>long</c>/<c>bool</c>, built as a
/// small AST and rendered to C#. Every expression reads a variable, so none is a compile-time constant
/// (a constant <c>checked</c> overflow or division by zero would be a compile error); literals appear
/// only as right operands, and never as a zero divisor.
/// </summary>
public static class LoweringOracleGen
{
    ...
    public static Gen<OracleInput> Input { get; } =
        Gen.Select(Int, Int, Long, Long, Gen.Bool, static (a, b, c, d, e) => new OracleInput(a, b, c, d, e));
    ...
    private static Gen<Stmt> StmtGen(Type returnType, int depth)
    {
        ...
        if (depth == 0)
        {
            return Gen.Frequency((6, assign), (1, exit));
        }
        ...
        return Gen.Frequency((4, assign), (2, branch), (1, exit));
    }
    ...
    private static Gen<Expr> ExprGen(Type type, int depth)
    {
        ...
        return type == typeof(bool) ? Bool(depth, leaf) : Number(type, depth, leaf);
    }
    ...
    private static Gen<Expr> Bool(int depth, Gen<Expr> leaf)
    {
        ...
        return Gen.Frequency((2, leaf), (3, relation), (2, logic), (1, not));
    }

    private static void Render(ImmutableArray<Stmt> block, StringBuilder text, int indent)
    {
        ...
        foreach (Stmt statement in block)
        {
            switch (statement)
            {
                ...
                case If branch:
                    ...
                    Render(branch.Then, text, indent + 1);
                    text.Append(pad).Append("}\n").Append(pad).Append("else\n").Append(pad).Append("{\n");
                    Render(branch.Else, text, indent + 1);
                    text.Append(pad).Append("}\n");
                    break;
            }
        }
