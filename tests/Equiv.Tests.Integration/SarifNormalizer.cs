...
/// every string that carries <paramref name="sampleDir"/> as an absolute prefix becomes sample-relative: a
...
internal static class SarifNormalizer
{
    ...
    {
        string prefixUri = new Uri(sampleDir.Replace('\\', '/') + "/").AbsoluteUri;
        ...
        foreach (JValue value in root.Descendants().OfType<JValue>().Where(static v => v.Type == JTokenType.String).ToList())
        {
            ...
            string relative = text
                .Replace(prefixUri, "file:///", StringComparison.Ordinal)
                .Replace(prefixSlash, string.Empty, StringComparison.Ordinal).Replace(prefixBackslash, string.Empty, StringComparison.Ordinal);
