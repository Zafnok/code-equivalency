...
/// <c>null.&lt;To&gt;</c>, not tied to the operand's, and one <c>typeof.&lt;T&gt;</c> input per closed type read by
/// <c>typeof(T)</c> (ticket P2-002), which is never null and adds no trace event. Each is created once, on first use, and they become parameters ordered by name, so both
...
/// <c>cast.*</c>, <c>length.*</c> and <c>typeof.*</c> are <see cref="IrParameterKind.In"/>, because nothing the body does changes them.
