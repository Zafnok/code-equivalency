...
SonarQube server. Add a scanner step to CI that runs on every PR, feeds it the cobertura
report `build.ps1` already produces, and reports the default "Sonar way" quality gate
(new-code coverage, duplication, maintainability/reliability/security ratings) as a PR
check. Start non-blocking (`continue-on-error: true`), same rollout pattern used for
Stryker in M0-004, then promote to a required check once calibrated against a few real
PRs.

## Why
- SonarQube Cloud's free tier covers private repos up to 50,000 lines of code, includes
  PR analysis and the new-code quality gate, up to 5 users, and C# support — this repo
  is well under that ceiling at MVP stage.
