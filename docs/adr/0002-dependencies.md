...
| Microsoft.Z3 | 4.12.2 | Verify.Z3 | MIT (confirmed 2026-09-21 from the published 4.12.2 nuspec: `<license type="expression">MIT</license>`, so `tools/licence-check` reads it as SPDX with no `policy.json` exception; upstream Z3Prover/z3 is MIT too) | official bindings. Ships `libz3` for win-x64 and osx-x64 only (no `runtimes/linux-x64`, found in M3-001): the Linux CI legs take `libz3.so` from the PyPI `z3-solver==4.12.2.0` manylinux wheel, pinned by SHA-256 (not a NuGet package), and M3-004's linux-x64 publish must bundle one. 4.12.2 is still the newest version on nuget.org as of 2026-09-21, so the package lags upstream (last push 2023) but the API is stable. Escape hatch: drop-in newer libz3 from GitHub releases |
