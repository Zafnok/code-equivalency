...
## Notes
- Decision: `Update` is split in two. The shared one reads the target, applies an `operate` function to the value read and writes its result; `operate` evaluates the right operand itself, so it comes after the read, and branches on its flags, so they come before the write. The bitvector compound and `++`/`--` keep their promote, operate, narrow path as one such function, and the pure paths pass `Apply` of the catalogued entry or `Operator` of the user-defined method. Alternatives: a second copy of the read and write. Rule: 3.
- Decision: criterion 5's identity `InConversion` and `OutConversion` are checked through the types, with M4-002's exactness test (`IsExact`): the method takes exactly the target's and the operand's types and returns the target's, which holds only when both conversions are identities. It also rejects a C# 14 instance `operator +=` or `operator ++`, which takes one operand fewer and returns `void`. On the catalogue path the entry is found only when the target's and the operand's types are the same `float`, `double` or `decimal`, which again means identity conversions. Alternatives: test `InConversion.IsIdentity` and `OutConversion.IsIdentity` as well (branches no input can reach). Rule: 3.
- Decision: the `1` of `++`/`--` on `float`, `double` and `decimal` is `TypeMapper.Constant(type, 1)`, the element hashed from the text `1`, which is the element the literal `1f`, `1d` or `1m` lowers to (`1.0m` is another element, as it is another `decimal`). Rule: 1.
- Decision: the lowering oracle's increments are `decimal` expression leaves `(m++)`, `(++m)`, `(m--)` and `(--m)`, so postfix and prefix values are observed; `m op= e` is a statement. The test's `DecimalOracle` answers the element of `1m` with `1m`. The test asserts a `dec.add` whose target is the variable `m`, which only a compound assignment or an increment writes, since the generator never assigns `m`. Rule: 3.
- Decision: `PureLoweringTests.DecimalIncrementHasTheOverflowFlag` pins criterion 4's `decimal` `++` flag on its own, beside `IncrementIsAddingTheLiteralOne`. Rule: 3.
- Deviation: `tests/Equiv.Frontend.CSharp.Tests/Lowering/IrLowererTests.cs` is not in Files. Its `CompoundAssignmentToAnUnsupportedTargetIsOpaque` pinned `d += 1` and `d++` on `double`, and a static user-defined `int += S`, as opaque, which is what this ticket changes; those three rows are removed (the new `PureLoweringTests` cover them). `PureLoweringTests.LiftedOrWidenedOperatorsStayOpaque` loses its `a += 1.5` row for the same reason.
- Deviation: `samples/business-layer/expected.sarif.json` is not in Files, but `SamplesEndToEndTests` compares it and the new method adds a result. It gains `Discounted`'s Equivalent result (`proofMethod: bounded`), the census counts below, and the line counts (legacy 134 to 139, modern 130 to 135). Before merging M4-008 it also added `OrderLine::get_Quantity()` and `get_UnitPrice()` to `Subtotal`'s `assumedCallees`; after the merge `main` already lists them.
- Counts that move in `BusinessLayerCensusSnapshot`: procedures 28 to 29 on each side, matched pairs 28 to 29 (the new `Discounted`); pairs without opaque 22 to 24 on top of M4-008 (8 to 10 before it; `Subtotal` and `Discounted`); the `CompoundAssignment` reason (1 and 1, `Subtotal`) leaves; changed pairs 4 to 5 and changed pairs without opaque 3 to 4 (`Discounted`, whose bound fingerprints differ), so the empty reason set goes from 3 to 4. Congruent pairs (24) and whole-body opaque pairs (2 after M4-008) do not move.
- Surprise: `float f; f += d` with a `double d` does not compile (CS0266: the operand must convert implicitly to the target), so a catalogued compound assignment with a non-identity conversion cannot be written. Criterion 6's conversion case is a user-defined `+` reached through implicit user-defined conversions (`Wrapper += Money`).
- Note: unlike M4-002's run, the `business-layer` sample loads on Linux here, so `PureOperatorTests` and `BusinessLayerCensusSnapshot` ran locally and pass (after `tools/z3-feed/fetch.ps1` filled `.z3-feed`). `SamplesEndToEndTests` fails on Linux for every sample only because artifact URIs are written `legacy/X.cs` rather than `file:///legacy/X.cs`; the business-layer snapshot was taken from the Linux output with that form restored, and every other sample's output matched its snapshot byte for byte after the same substitution.
- Note: merging `main` after M4-008, which lowers an auto-property as reads and writes of its backing field, turned the property target in `ACompoundOverflowLeavesTheTargetUnwritten` and `IrLowererSnapshotTests.PureCompoundAssignment` into field accesses. Both now use a property whose accessors have bodies (`get { return F; } set { F = value; }`), which stays a getter call and a setter call, so criterion 3 is still about the accessors. The snapshot text is unchanged.
