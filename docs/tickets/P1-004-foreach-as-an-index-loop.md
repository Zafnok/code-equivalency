...
(`for (int i = 0; i < a.Length; i++) { T x = a[i]; ... }`) instead of one whole-body
`IrOpaque("foreach-enumerator")`.
...
2. The pattern is recognised from the CFG and the operation tree together, never from syntax,
   and a `foreach` whose shape does not match stays `IrOpaque("foreach-enumerator")`.
3. `foreach` over anything that is not an array stays `IrOpaque("foreach-enumerator")`.
