...
- CI (`gates (windows-latest)`) failed on the first push: `EndpointDiscoverySampleTests` couldn't load `webapi-basic/legacy` (CS0246/CS0234 for the WebApi.Core attribute types) on a fresh checkout, even though the same run's local reproduction had passed — because I had manually restored the sample with VS Build Tools' `MSBuild.exe` earlier while investigating the toolchain note above, and that `obj/` cache (gitignored) papered over the real gap locally. `build.ps1 -Integration` now restores every `samples/**/*.csproj` explicitly (`MSBuild.exe -t:Restore` for `legacy/`, `dotnet restore` for `modern/`) before the test step, since `samples/**` sits outside `Equiv.slnx` and MSBuildWorkspace itself never restores. Reproduced the failure locally by deleting `samples/webapi-basic/**/obj` and `bin`, confirmed the fix from that same clean state.
