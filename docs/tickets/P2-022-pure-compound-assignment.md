...
- Deviation: `samples/business-layer/expected.sarif.json` is not in Files, but `SamplesEndToEndTests` compares it and the new method adds a result. It gains `Discounted`'s Equivalent result (`proofMethod: bounded`), the census counts below, the line counts (legacy 134 to 139, modern 130 to 135), and `Subtotal`'s congruent result now lists `OrderLine::get_Quantity()` and `get_UnitPrice()` in `assumedCallees`: its `+=` operand was inside the `CompoundAssignment` opaque and is now lowered, so the getters are calls the IR makes.
- Counts that move in `BusinessLayerCensusSnapshot`: procedures 28 to 29 on each side, matched pairs 28 to 29 (the new `Discounted`); pairs without opaque 8 to 10 (`Subtotal` and `Discounted`); the `CompoundAssignment` reason (1 and 1, `Subtotal`) leaves; changed pairs 4 to 5 and changed pairs without opaque 3 to 4 (`Discounted`, whose bound fingerprints differ), so the empty reason set goes from 3 to 4. Congruent pairs stay 24 and whole-body opaque pairs 16.
