...
## Deliverables
- [ ] `build.ps1` deletes `TestResults/` before the test step. Today reports from earlier
...
- [ ] `check-coverage` fails when any `src/` assembly discovered by `SrcAssemblyDiscovery`
...
- [ ] `check-coverage` report table shows `covered/valid` counts next to each rate so a
      `0/0 = 100%` placeholder is visible to a reviewer.
- [ ] `src/Equiv.Cli/Program.cs` becomes an explicit `internal static class Program` with
...
- [ ] `.editorconfig`: `insert_final_newline = true` for `[*.cs]`; run `dotnet format`.
