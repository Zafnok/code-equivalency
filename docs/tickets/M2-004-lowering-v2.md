...
- Decision: a folded `switch` keeps the CFG *branch* of each case and of the fall-out, not the block, and lowers each through the same `Destination` every unfolded branch uses, so an edge that leaves a `try` runs its `finally`. Alternatives: refusing to fold a chain whose successors cross a `finally` (loses the `IrSwitch` acceptance criterion 2 asks for, on exactly the bodies most worth folding). Rule: 1.
  - Review fix (PR #30): this line previously recorded the refuse-to-fold alternative, but `SwitchChains.Read` only ever checked `Semantics: Regular` -- it never looked at `FinallyRegions`, so nothing implemented the guard. A folded chain jumped straight to `blockIds[...]`, and the fall-out edge skipped the `finally` outright: `try { if (x==1) r=1; else if (x==2) r=2; else if (x==3) r=3; } finally { r += 10; }` returned 0 for `x = 4` instead of 10. Silent wrong IR on a path with no `IrOpaque`, so ADR 0014 does not catch it -- a candidate false Equivalent. `IrLowererTests.AFoldedSwitchRunsTheFinallyOnEveryEdge` pins it.
...
- Review fix (PR #30): `Raise` resolved a matching `catch` as `blockIds[handler.FirstBlockOrdinal]`, but `Copy` swaps `blockIds` for the copy's own map while a `finally` is being re-lowered, and a `catch` outside the `finally` is not in it. Any `finally` that can raise -- a call, a division, `checked` -- nested in the `try` of a `try`/`catch` threw `KeyNotFoundException` out of `IrLowerer`, which is documented never to throw on unsupported input. `Handler` now falls back to the main pass's map, keeping the copy's own map first so that a `catch` nested *inside* the `finally` still resolves there. `IrLowererTests.AThrowInsideAFinallyGoesToACatchOutsideIt`, `.ACallThatThrowsInsideAFinallyGoesToACatchOutsideIt` and `.AThrowInsideAFinallyGoesToACatchInsideThatFinally` pin all three shapes.
- Note: 100% line and branch coverage passed over both of the bugs above. Every line involved was executed by some test; only the combination of an exception region with a `finally` copy context was not. The gate is worth less than its number here, and the next ticket that touches regions should add shape-level cases rather than trust it.
- Note: two modelling gaps this ticket leaves open, now written into VERIFICATION-MODEL.md section 2 so M3-001 does not assume otherwise: an `IrCall` does not havoc any `field.*` map, and `array.<v>` is keyed per array *variable*, so aliasing is unmodelled. Both are what the acceptance criteria asked for; neither is sound in general.
...
- Note: `IrLowerer.cs` is now about 1000 lines. The ticket's size guard is per construct, and no single construct here is near 150 lines (the biggest, `finally` duplication, is about 60), but the class is a candidate for splitting the heap and exception work out in a later ticket.
  - Review outcome (PR #30): that ticket is now P1-003, which also replaces the four fields `Copy` swaps with an explicit `LoweringContext` parameter, so the second review fix above becomes a compile error rather than a fallback. The `foreach` ticket renumbered to P1-004 and follows it, because it adds to both of the paths P1-003 extracts.
- Note: the integration suite runs on this Windows box (`./build.ps1 -Integration`), so acceptance criterion 8 was checked against the real samples, not only against snippets.
